...
const insertFeedback = async (payload: Feedback): Promise<Feedback> => {
  ...
  if(feedbackExist) throw new ApiError(httpStatus.CONFLICT,'Feedback already exist!')
...
const updateFeedback = async (id:string, payload: Feedback): Promise<Feedback | null> => {
  ...
  if(!feedbackExist)
  throw new ApiError(httpStatus.NOT_FOUND, 'Feedback not exists')
...
const deleteFeedback = async (id:string): Promise<Feedback | null> => {
  ...
  const feedback = await prismaClient.feedback.delete({
    where: {
      id
    }
  })
